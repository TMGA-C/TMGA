...
void ConfigParameter_TMGA(Orthogonal orthogonal) {// -xy
    Parameter_TMGA.NumOfChrInSubPop = Tasks.size() * orthogonal.SubPopSizeFac;                                    //set the subpopulation size
    if (Parameter_TMGA.NumOfChrInSubPop % 2 == 1) {
        ++Parameter_TMGA.NumOfChrInSubPop;
    }
    Parameter_TMGA.NumOfSubPop = orthogonal.NumOfSubPopM;                                                        //set the number of subpopulations
    Parameter_TMGA.interval = orthogonal.ExchangeFactor;                                                        // set the exchange interval
    Parameter_TMGA.NumOfEliteOfPop = ceil(Parameter_TMGA.NumOfChrInSubPop * orthogonal.ElistRate);           //set the number of elitist for exchange -xy
    Parameter_TMGA.MutationRate_TMGA = orthogonal.MutationRate;                                                 //set the mutation rate
    Parameter_TMGA.TrmThresholdOfStg1 = ceil(ModelScale * orthogonal.TerFacForSta1 /Parameter_TMGA.NumOfChrInSubPop/Parameter_TMGA.NumOfSubPop);  //set the termination condition of stage 1
}
