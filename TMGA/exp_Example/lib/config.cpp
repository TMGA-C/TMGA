...
void ReadFile(string XmlFile, string RscAlcFile) {
    ...
    Resource Rsc_0 = Resource(0, 1, 1);//-w
    Resource Rsc_1 = Resource(0, 1.5, 1.5);
    Resource Rsc_2 = Resource(0, 2, 2);
...
void ConfigParameter_TMGA() {// -xy
    Parameter_TMGA.NumOfChrInSubPop = Tasks.size()*1.5;                                   //set the subpopulation size
    ...
    Parameter_TMGA.interval = 14;                                                       // set the exchange interval/0.8
    ...
    Parameter_TMGA.TrmThresholdOfStg1 = ceil(8 * ModelScale / Parameter_TMGA.NumOfChrInSubPop / Parameter_TMGA.NumOfSubPop);  //set the termination condition of stage 1
}
