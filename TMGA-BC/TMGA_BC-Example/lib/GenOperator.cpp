        ...
        if(IsFrw) {                              //forward-loading
            for (int j = 0; j < Tasks[TaskIndex].parents.size(); ++j) {  //calculate the ready time of the task
                ...
                if(RscIndex != ParentRsc) {
                    double TT = ParChildTranFileSizeSum[ParentTask][TaskIndex] / VALUE  / (XY_MIN(Rscs[RscIndex].bw, Rscs[ParentRsc].bw));
            ...
            for (int j = 0; j < Tasks[TaskIndex].children.size(); ++j) {
                ...
                if(RscIndex != ChildRsc) {
                    double TT = ParChildTranFileSizeSum[TaskIndex][ChildTask] / VALUE  / (XY_MIN(Rscs[RscIndex].bw, Rscs[ChildRsc].bw));
    ...
    for (int i = 0; i < comConst.NumOfTsk; ++i) {
        ...
        if(Tasks[i].parents.size() !=  0){
            for (int j = 0; j < Tasks[i].parents.size(); ++j) {
                ...
                if(ParRsc != RscIndex){
                    TransferTime[parent][i] = ParChildTranFileSizeSum[parent][i] / VALUE  / XY_MIN(Rscs[RscIndex].bw,Rscs[ParRsc].bw) ;
                }
            }
        }
